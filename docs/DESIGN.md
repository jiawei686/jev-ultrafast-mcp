...
| Confirmation rules | **Every op that clicks** whose accessible name matches `pay now`, `delete account`, `unsubscribe`, … returns `needs_confirmation` instead of executing; re-send the op with `"confirm": true`. Attached to clicking rather than to the op named `click`: `toggle` presses the control too, and the rail was written into one of `_do_click`'s two callers. The role that gates the rule (a checkbox named "Delete account" is not a click this stops for; a button is) is read from the observed element, not from the op — a `"role"` in the request is a claim about the target, and it used to be able to lift the rail outright |
| Secret fields | Password inputs stay **usable** (jev drops them, so it cannot log in) but their value is never serialised: rendered as `«hidden»`, required `confirm` to type into, and never written into a macro. The rail is the same **union** the mask is — the page's own `secret` flag *or* the name/role patterns — because a rail narrower than the mask lets through what the table hides: reading only the label left a password field with no accessible name ungated. There are two ops that can put text into one and only one of them can honour that: `type` takes a ref, asks, and stores `{{secret}}`, while `keys` with a bare single character goes out as `Input.insertText` into whatever has focus and has no way to record a placeholder — so it is **refused** on a sensitive field, with the refusal naming `type` |
